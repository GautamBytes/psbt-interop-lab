...
The PSBTv2 baseline now enforces the canonical omission of empty final scriptSig fields and names
strict-parser differences around undefined `PSBT_GLOBAL_TX_MODIFIABLE` bits. These findings are
bounded to the affected implementations; completed transactions still have to pass Bitcoin Core
policy on isolated regtest.
