...
host, confidentiality of local artifact metadata, and bounded runtime and CI execution.

The controls fail closed on malformed adapter messages, changed round-trip PSBT bytes, mismatched
Bitcoin Core RPC replies, an unexpected Core version, and inconsistent replay checkpoints. They do
not make this a production signer. Adapter identity is a compatibility self-report rather than
attestation, and hashes stored in a mutable artifact manifest establish internal consistency rather
than authenticity.
...
publish a release, attest an image, or make CI a runtime security boundary for the local CLI.
...
- `verifyReplay` in `src/runner/replay.ts` checks bounded checkpoint/facts consistency offline. It
  does not rerun adapters or independently recompute the recorded scenario outcome.
...
| --- | --- | --- |
| Proof outcome and checkpoints | A PASS must correspond to the expected generated-regtest transitions, byte-preserving round trips, Core finalization, and policy acceptance. | `runProof`, `assertByteIdenticalRoundtrip`, `CoreRpc.call`, and `ArtifactRun` fail closed. Adapter provenance is not attested. |
...
| Artifact metadata confidentiality | PSBT checkpoints, scripts, UTXOs, and implementation metadata should not be exposed through reports or broad local permissions. | `ArtifactRun` uses `0700` directories and `0600` files; reports redact PSBT-shaped and secret-like values. Files are not encrypted from the trusted host account. |
...
| CI runner availability | Untrusted pull-request code should not consume unbounded hosted compute. | `.github/workflows/ci.yml` uses job timeouts and concurrency cancellation. Build scripts retain network access until a job ends. |
| CI and downstream release integrity | CI results should come from the checked-out revision and pinned workflow actions without write credentials or secrets. | Read-only permissions, `persist-credentials: false`, exact action commits, lockfiles, and image/dependency hashes support reproducibility. The workflow does not publish releases or provide artifact/image attestation. |
...
- Corrupted local artifact files, including truncation, digest mismatch, final-component symlinks,
  oversized files, path traversal records, or a manifest with excessive checkpoints.
...
| Adapter compatibility | `src/scenarios/contracts.ts`: `assertAdapterHello` | Pins self-reported name, version, source revision, operations, and PSBTv0 support | Self-report and shaped `artifactDigest` are compatibility metadata, not image attestation. |
...
| Replay | `src/runner/replay.ts`: `verifyReplay` | Canonical root, contained relative paths, bounded manifest/files, 1,000-checkpoint cap, descriptor-based regular-file and digest checks | `O_NOFOLLOW` protects only the final path component; mutable manifest hashes are not authenticity. |
...
1. **False PASS through adapter self-report.** An adapter can return `status: ok`, claim the expected
   ...
   PSBT. Later signing/finalization claims are checked by Core. A malicious binary can still spoof
   identity metadata, so the result is compatibility evidence, not provenance attestation.
...
5. **Mutable artifact tampering.** Replay detects a checkpoint or facts file that no longer agrees
   with the manifest. An editor controlling the artifact directory can instead replace the PSBT,
   facts, manifest hashes, scenarios, and outcome together. No external signature or immutable root
   exists, and `O_NOFOLLOW` does not protect intermediate path components, so authenticity is not
   claimed.
6. **CI compute abuse.** Pull-request code can execute package, compiler, and test commands on hosted
   runners. Read-only permissions, absent secrets, ephemeral jobs, timeouts, concurrency
   cancellation, and omission of the Docker proof reduce impact. Job network access and the hosted
   platform remain available until termination, so residual low-cost abuse is possible.

## 8. Threat Model Table

| ID | Asset | Preconditions | Abuse path | Impact | Controls | Residual risk | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| TM-001 | Proof outcome and implementation compatibility | Adapter is buggy, stale, or lies in JSONL output | Self-reported identity or `byteIdentical` creates a false PASS | Incorrect interoperability conclusion | `AdapterProcess.request` validates schema/ID/bounds; `assertAdapterHello` pins compatibility fields; `assertByteIdenticalRoundtrip` independently parses and compares bytes; Core finalizes and policy-checks results | A malicious binary can spoof expected identity and digest strings; provenance is not attested. Under trusted local images, practical impact is low. | Mitigated, low residual risk |
...
| TM-005 | Artifact integrity and confidentiality | Local artifacts are corrupted or an actor can edit the artifact directory | Replace checkpoints, facts, manifest hashes, recorded scenarios, or path components | Misleading replay result or metadata disclosure | `ArtifactRun` uses contained atomic private writes; `verifyReplay` uses bounded regular-file descriptor reads, final-component `O_NOFOLLOW`, structural parsing, and digest comparisons | Hashes and outcomes share one mutable directory and have no signature or immutable trust root. The trusted account can rewrite a self-consistent run. | Accepted, low risk under trusted-local scope |
| TM-006 | CI availability and check integrity | Untrusted pull-request code executes in hosted jobs | Consume compute/network, alter test behavior, or seek credentials | CI delay/cost or misleading PR signal | `contents: read`, no workflow secrets, `persist-credentials: false`, pinned actions, ephemeral runners, job timeouts, concurrency cancellation; Docker proof skipped for pull requests | Build code retains network and compute until timeout; GitHub runner isolation and registries are externally trusted. | Mitigated, low residual risk |
...
compute abuse. A CI PASS is build evidence for that revision, not release or image attestation.
...
5. Subsequent signed/finalized states are passed to `CoreRpc.call` for `finalizepsbt` and
   `testmempoolaccept`, tying the result to the pinned local Core contract.
...
2. `readRegularFile` opens the final path using `O_RDONLY | O_NOFOLLOW`, uses the same `FileHandle` to
   check regular-file type and size, reads it, and closes it. `O_NOFOLLOW` covers the final component
   only; intermediate directories remain part of the trusted local artifact boundary.
3. `parseManifest` checks the minimal schema and `verifyReplay` rejects more than 1,000 checkpoints.
4. Each relative checkpoint path is constrained beneath the canonical root. The PSBT final component
   is opened without symlink following, bounded, line-framed, reparsed by `extractWireFacts`, and
   compared with the manifest SHA256. The facts file is opened similarly and its SHA256 field must
   agree.
5. Replay returns the manifest's recorded outcome and scenarios without launching adapters or
   recomputing scenario semantics. It proves internal checkpoint consistency at read time, not the
   authenticity of a mutable directory or the identity of the original producer.
...
4. The `docker-proof` job condition permits only `workflow_dispatch` or `refs/heads/main` and is false
   ...
   boundary.
5. The runner is discarded after the job. Residual network/compute abuse before timeout is accepted
   as low within the no-secret, read-only, ephemeral CI scope.
