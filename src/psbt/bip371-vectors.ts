// Generated from Bitcoin BIP371 at commit b289d016b99c81527623c10e995e0318f744ebf3.
// Source: https://github.com/bitcoin/bips/blob/b289d016b99c81527623c10e995e0318f744ebf3/bip-0371.mediawiki

export interface Bip371Vector {
  readonly id: string;
  readonly title: string;
  readonly base64: string;
}

export const BIP371_SOURCE_SHA256 =
  "f7bde92b1de04c0286c678930dd11fdafbfaa7e72767038e44e12fd4d4b31091";
export const BIP371_CORPUS_SHA256 =
  "6afe6b443edc7eeab2d424b743d048fc9493bd4728910bc199ca9837433a57a0";

export const BIP371_VALID_VECTORS = [
  {
    id: "valid-01",
    title: "PSBT with one P2TR key only input with internal key and its derivation path",
    base64:
      "",
  },
  {
    id: "valid-02",
    title:
      "PSBT with one P2TR key only input with internal key, its derivation path, and signature",
    base64:
      "",
  },
  {
    id: "valid-03",
    title: "PSBT with one P2TR key only output with internal key and its derivation path",
    base64:
      "",
  },
  {
    id: "valid-04",
    title:
      "PSBT with one P2TR script path only input with dummy internal key, scripts, derivation paths for keys in the scripts, and merkle root",
    base64:
      "",
  },
  {
    id: "valid-05",
    title:
      "PSBT with one P2TR script path only output with dummy internal key, taproot tree, and script key derivation paths",
    base64:
      "",
  },
  {
    id: "valid-06",
    title:
      "PSBT with one P2TR script path only input with dummy internal key, scripts, script key derivation paths, merkle root, and script path signatures",
    base64:
      "",
  },
] as const satisfies readonly Bip371Vector[];

export const BIP371_INVALID_VECTORS = [
  {
    id: "invalid-01",
    title:
      "PSBT With PSBT_IN_TAP_INTERNAL_KEY key that is too long (incorrectly serialized as compressed DER)",
    base64:
      "",
  },
  {
    id: "invalid-02",
    title: "PSBT With PSBT_IN_TAP_KEY_SIG signature that is too short",
    base64:
      "",
  },
  {
    id: "invalid-03",
    title: "PSBT With PSBT_IN_TAP_KEY_SIG signature that is too long",
    base64:
      "",
  },
  {
    id: "invalid-04",
    title:
      "PSBT With PSBT_IN_TAP_BIP32_DERIVATION key that is too long (incorrectly serialized as compressed DER)",
    base64:
      "",
  },
  {
    id: "invalid-05",
    title:
      "PSBT With PSBT_OUT_TAP_INTERNAL_KEY key that is too long (incorrectly serialized as compressed DER)",
    base64:
      "",
  },
  {
    id: "invalid-06",
    title:
      "PSBT With PSBT_OUT_TAP_BIP32_DERIVATION key that is too long (incorrectly serialized as compressed DER)",
    base64:
      "",
  },
  {
    id: "invalid-07",
    title:
      "PSBT With PSBT_IN_TAP_SCRIPT_SIG key that is too long (incorrectly serialized as compressed DER)",
    base64:
      "",
  },
  {
    id: "invalid-08",
    title: "PSBT With PSBT_IN_TAP_SCRIPT_SIG signature that is too long",
    base64:
      "",
  },
  {
    id: "invalid-09",
    title: "PSBT With PSBT_IN_TAP_SCRIPT_SIG signature that is too short",
    base64:
      "",
  },
  {
    id: "invalid-10",
    title: "PSBT With PSBT_IN_TAP_LEAF_SCRIPT Control block that is too long",
    base64:
      "",
  },
  {
    id: "invalid-11",
    title: "PSBT With PSBT_IN_TAP_LEAF_SCRIPT Control block that is too short",
    base64:
      "",
  },
] as const satisfies readonly Bip371Vector[];
