...
export function discoverReceiverOutputs(
  ...
  for (let k = 0; k < doc.outputCount - 1; k++) {
    ...
    if (matches.length > 1)
      throw new Error("Duplicate recipient output outside the bounded fixture");
    const match = matches[0];
    if (match?.location.kind !== "output") break;
    found.push({
...
export function createReceiverPsbt(
  ...
  if (
    ...
    new Set(outputs.map((o) => o.scriptHex)).size !== outputs.length ||
    outputs.some(
      (o) =>
        ...
        o.amountSats <= 330n ||
        o.amountSats > 2_100_000_000_000_000n,
    )
  )
    throw new Error("Invalid discovered fixture outputs");
  ...
  const bytes = Buffer.concat([
    ...
    pair(2, u32(2)),
    pair(3, u32(0)),
    pair(4, Buffer.from([outputs.length])),
