...
use psbt_lab_musig2_adapter::{
    ADAPTER_PROTOCOL, FixtureCommitments, Musig2Adapter, SignerIdentity,
};
...
fn artifact_digest() -> String {
    let bytes = std::env::current_exe()
        ...
        .unwrap_or_else(|| b"psbt-lab-musig2-adapter".to_vec());
...
fn main() -> io::Result<()> {
    ...
    let selector = std::env::var("PSBT_LAB_MUSIG2_SIGNER").map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
            "PSBT_LAB_MUSIG2_SIGNER must be 1 or 2",
        )
    })?;
    let identity = SignerIdentity::from_selector(&selector)
        .map_err(|message| io::Error::new(io::ErrorKind::InvalidInput, message))?;
    let commitments = FixtureCommitments::from_json(
        std::env::var("PSBT_LAB_FIXTURE_COMMITMENTS")
            .ok()
            .as_deref(),
    )
    .unwrap_or_else(|_| FixtureCommitments::invalid());
    let mut adapter = Musig2Adapter::new(identity, commitments);
    let stdin = io::stdin();
    ...
    while read_bounded_line(&mut reader, &mut line)? {
        let response = match serde_json::from_slice::<Value>(&line) {
            Ok(value) => adapter.handle_value(value, &digest),
            Err(_) => json!({
